...
CARDDEFS_OUT="$BASEDIR/src/CardDefs.xml"
...
rm -rf "$BASEDIR/src/Strings"
cp -rf "$HSDATA_DIR/Strings" "$BASEDIR/src"
find "$BASEDIR/src/Strings" -type f -not -name "GLOBAL.txt" | xargs rm
